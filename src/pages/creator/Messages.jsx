              ...
              conversations.map((conv) => (
                <button
                  ...
                  className={`w-full p-4 text-left hover:bg-gray-50 border-b border-gray-100 ${
                    conversationId === conv.id ? 'bg-green-100' : ''
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                      {conv.brand?.logoUrl ? (
                        <img src={conv.brand.logoUrl} alt="" className="w-full h-full rounded-full object-cover" />
                      ) : (
                        <span className="text-gray-500">{conv.brand?.companyName?.[0]}</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        <p className="font-medium text-gray-900 truncate">{conv.brand?.companyName}</p>
                        {conv.unreadCount > 0 && (
            ...
            <>
              ...
              <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center">
                  {activeConversation?.brand?.logoUrl ? (
                    <img src={activeConversation.brand.logoUrl} alt="" className="w-full h-full rounded-full object-cover" />
                  ) : (
                    <span className="text-gray-500">{activeConversation?.brand?.companyName?.[0]}</span>
                  )}
                </div>
                <div>
                  <p className="font-medium text-gray-900">{activeConversation?.brand?.companyName}</p>
