...
const CreatorLayout = () => {
  ...
  const getTierBadge = () => {
    ...
    const styles = {
      rising: { bg: 'bg-gray-100 text-gray-600', icon: TrendingUp },
      verified: { bg: 'bg-blue-100 text-blue-600', icon: Star },
      premium: { bg: 'bg-purple-100 text-purple-600', icon: Award },
      elite: { bg: 'bg-gradient-to-r from-amber-400 to-orange-500 text-white', icon: Sparkles },
    };
    return styles[tier] || styles.rising;
  };
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-72 bg-slate-900 transform transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 py-6 px-4 overflow-y-auto">
            ...
            {/* Tier Progress Card */}
            {profile?.tier !== 'elite' && (
              <div className="mt-8 p-4 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl">
                <div className="flex items-center gap-2 mb-2">
                  <TierIcon size={18} className="text-blue-200" />
                  <span className="text-sm font-semibold text-white capitalize">{profile?.tier || 'Rising'} Creator</span>
                </div>
                <p className="text-xs text-blue-100 mb-3">
                  Level up to unlock lower fees and premium features
                </p>
                <Link
                  to="/creator/earnings"
          ...
          <div className="p-4 border-t border-slate-800">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/50">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-pink-400 to-purple-500 flex items-center justify-center flex-shrink-0">
                {profile?.profileImage ? (
                  <img src={profile.profileImage} alt="" className="w-full h-full rounded-xl object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <TierIcon size={12} className={profile?.tier === 'elite' ? 'text-amber-400' : 'text-gray-400'} />
                  <span className="text-xs text-gray-400 capitalize">{profile?.tier || 'Rising'} Creator</span>
                </div>
              </div>
            </div>
            <button
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden sm:block text-left">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{profile?.tier || 'Rising'} Creator</p>
                  </div>
